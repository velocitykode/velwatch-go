...
// The exception used to come from a request.failed listener (onRequestFailed).
// Main records errors through the framework's error reporter instead
// (reporter.go, onErrorReported), and onRequestFailed no longer exists, so
// this test drives the same finding through the reporter: the request record
// (request.handled) and the exception the error handler reports for that
// request, followed onto the OTLP wire where the collector reads them.
func TestExceptionIsChildOfRequestNotSameSpan(t *testing.T) {
	collector := testCollector()
	l := NewListeners(collector, events.NewDispatcher(), "svc", 1.0)

	// Hex ids, so they survive decodeID on the wire unchanged.
	const traceID = "0123456789abcdef0123456789abcdef"
	const reqSpan = "0123456789abcdef"

	if err := l.onRequestHandled(&router.RequestHandled{
		Context:    context.Background(),
		Method:     "GET",
		Path:       "/boom",
		StatusCode: 500,
	...
	newErrorReporter(l).Report(context.DeadlineExceeded, &contract.ErrorContext{
		Method:  "GET",
		URL:     "/boom",
		TraceID: traceID,
		SpanID:  reqSpan,
	...
	ev := got[0]
	if ev.SpanID == reqSpan {
		t.Error("exception must not share the request's span ID (invalid OTLP: two rows, one span)")
	}
	if ev.ParentID == nil || *ev.ParentID != reqSpan {
		t.Errorf("exception ParentID = %v, want the request span %q", ev.ParentID, reqSpan)
	}
	if ev.TraceID != traceID {
		t.Errorf("exception TraceID = %q, want the request's trace %q", ev.TraceID, traceID)
	}
	...
	// On the wire the exception is its own envelope span under the request
	// span, which is what the collector keys on to store one exception record
	// with this span's own id and parent.
	var request *Event
	for _, e := range getEvents(collector) {
	...
	reqWire, exWire := eventToSpan(request), eventToSpan(ev)
	if v := attrMap(exWire.Attributes)[attrExceptionEnvelope]; !v.GetBoolValue() {
		t.Errorf("exception span %s = %v, want true", attrExceptionEnvelope, v)
	}
	if string(exWire.SpanId) == string(reqWire.SpanId) {
		t.Error("exception span id equals the request span id on the wire")
	}
	if string(exWire.ParentSpanId) != string(reqWire.SpanId) {
		t.Errorf("exception parent span id = %x, want the request span id %x", exWire.ParentSpanId, reqWire.SpanId)
	}
}
